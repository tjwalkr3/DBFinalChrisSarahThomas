...
select cp.seat_id,
	...
	p.price,
	cpp.quantity
from airline_booking2.product p
inner join airline_booking2.concession_purchase_product cpp 
on (p.id = cpp.product_id)
inner join airline_booking2.concession_purchase cp 
on (cpp.concession_purchase_id = cp.id)
inner join airline_booking2.payment p2 
...
from airline_booking2.concession_purchase cp
inner join airline_booking2.seat s
on (cp.seat_id = s.id) 
...
inner join airline_booking2.passenger p 
...
with with_boarding_pass as (
    ...
    from airline_booking2.passenger p
    inner join airline_booking2.reservation r 
